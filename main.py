...
### Start
    ...
    ### Start    
        ...
        if 'код' in tokens or 'кот' in tokens:
            message = ("Код переработки — это треугольник из стр`елок с цифрой внутри. Цифры обозначают материал, из которого сделан предмет")
            ...
            message = ('Идентификатор переработки — это буквы на русском или латинице, расположенные под треугольником из стр`елок с цифрой внутри')
